...
import { TableActions } from '@/shared/components/table-actions';import { getModuleActions } from '@/shared/lib/module-actions-config';
export function TracePage() {
  const [items, setItems] = useState<ActivityListItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isOpen, setIsOpen] = useState(false);
  const [search, setSearch] = useState('');
  const [editingItem, setEditingItem] = useState<ActivityListItem | null>(null);  const moduleActions = getModuleActions('trace');
  const fetch = async () => {
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-slate-800/50 border-slate-700/50">
        ...
        <CardContent className="p-0">
          {isLoading ? (<div className="flex flex-col items-center justify-center py-20 gap-4"><Loader2 className="h-8 w-8 animate-spin text-cyan-500"/><p className="text-slate-400">Loading activities...</p></div>) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4"><Activity className="h-8 w-8 text-slate-500"/><p className="text-slate-400">No activities found</p></div>
          ) : (
      ...
      <CrudWizard isOpen={isOpen} onClose={handleCloseWizard} title={editingItem ? "Edit Activity" : "Log Activity"} mode={editingItem ? "edit" : "create"} fields={[
      ...
      ]} initialData={editingItem || {}} onSubmit={handleCreate} />
    </div>
  );
}
