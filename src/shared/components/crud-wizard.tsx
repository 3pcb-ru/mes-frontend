...
type Props<T extends Record<string, unknown>> = {
    ...
    mode?: 'create' | 'edit';
    fields: Field[];
    initialData?: T | DefaultValues<T> | undefined;
    onClose: () => void;
    onSubmit: (data: T) => Promise<void> | void;
};

export function CrudWizard<T extends Record<string, unknown>>({ isOpen, title, description, mode = 'create', fields, initialData = {} as T, onClose, onSubmit }: Props<T>) {
    const { t } = useTranslation();
    const form = useForm<T>({ defaultValues: initialData as any }); // RHF needs a bit of slack but the component prop is typed
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(submit)} className="space-y-5 pt-4">
                        ...
                        <DialogFooter className="pt-6 border-t border-slate-800/50">
                            <Button variant="ghost" type="button" onClick={onClose} className="text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
